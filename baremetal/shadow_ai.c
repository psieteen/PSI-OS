...
void learn_sequence(const char *from, const char *to) {
    ...
    if (sequence_count < MAX_SEQUENCES) {
        int j;
        for (j = 0; from[j] && j < MAX_CMDLEN-1; j++) {
            sequences[sequence_count].from[j] = from[j];
        }
        sequences[sequence_count].from[j] = '\0';
        for (j = 0; to[j] && j < MAX_CMDLEN-1; j++) {
            sequences[sequence_count].to[j] = to[j];
        }
...
void predict_next(const char *last_cmd) {
    if (last_cmd[0] == '\0') {
        uart_print("[Shadow AI] No previous command. Type something first!\n");
        return;
    }
    ...
    if (best_idx != -1) {
        ...
        uart_print("[Shadow AI] No pattern for '");
        uart_print(last_cmd);
        uart_print("' yet.\n");
    }
}
